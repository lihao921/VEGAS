...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, config: LanguageBindAudioConfig):
        ...
        if self.add_time_attn:  # this is not added in default!
        ...
        # print("config in modeling audio!-----------", config)
...
class LanguageBindAudio(CLIPPreTrainedModel):
    ...
    def __init__(self, config: LanguageBindAudioConfig):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        print('modeling_audio: Open convert_to_lora and resize_pos, which is open in official videollava')
        self.convert_to_lora()  # Must: close this when merging lora weights
        self.resize_pos(self.vision_model.embeddings, vision_config) # how about this? Always OPEN this

    def convert_to_lora(self):
        print("--------------If you are trying to merge from lora adapter.bin with original base model, you should not run this convert_to_lora!!!!!!!!!------------ ")
        print("--------------------converting to lora in model_audio.py--------------------")
        if self.lora_r == 0:
            return
        if self.add_time_attn:
