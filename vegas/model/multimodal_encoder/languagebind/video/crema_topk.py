...
def extract_frames_from_indicators(x, indicators):
    indicators = rearrange(indicators, "b d k -> b k d").to(x.dtype)
...
class ATPConfig:
    ...
    d_input: int = 628  # size of the input vision-language embeddings (e.g. CLIP-ViT-B32 is size 512)
...
# see https://github.com/StanfordVL/atp-video-language
...
class TopK_Selector(nn.Module):
    ...
    def init_t_embedding(self):
        init.normal_(self.t_embedding.weight, mean=0.0, std=self.sigma)
        if self.t_embedding.bias is not None:
            init.constant_(self.t_embedding.bias, 0)

    def init_embedding(self):
        init.normal_(self.embedding.weight, mean=0.0, std=self.sigma)
    ...
    def forward(self,
        ...
        x_inputs.append(x_txt.permute(1, 0, 2))
        ...
        x_logits = self.logits(self.dropout(x_encoded_v)).squeeze(-1)
        if self.training:
