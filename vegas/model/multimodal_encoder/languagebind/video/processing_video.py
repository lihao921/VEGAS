...
def load_and_transform_video(
    ...
    if video_decode_backend == 'pytorchvideo':
        ...
        duration = len(decord_vr)  # frames
        more_frames = True  # use when train or inference with a sampler
        if more_frames:
...
class LanguageBindVideoProcessor(ProcessorMixin):
    ...
    def __call__(self, images=None,context_length=77, return_tensors=None,text=None,  **kwargs):
        ...
        if text is not None:
            ...
            if isinstance(text, list):
                for q_text in text:
                    encoding = self.tokenizer(q_text, max_length=context_length, padding='max_length',
                                              truncation=True, return_tensors=return_tensors, **kwargs)
                    encoding_querys.append(encoding)
            else:
